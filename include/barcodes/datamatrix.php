...
// -----------------------------------------------------------------------------
	...
	protected function isCharMode($chr, $mode) {
		$status = false;
		$status = match ($mode) {
      ENC_ASCII => ($chr >= 0) AND ($chr <= 127),
      ENC_C40 => ($chr == 32) OR (($chr >= 48) AND ($chr <= 57)) OR (($chr >= 65) AND ($chr <= 90)),
      ENC_TXT => ($chr == 32) OR (($chr >= 48) AND ($chr <= 57)) OR (($chr >= 97) AND ($chr <= 122)),
      ENC_X12 => ($chr == 13) OR ($chr == 42) OR ($chr == 62),
      ENC_EDF => ($chr >= 32) AND ($chr <= 94),
      ENC_BASE256 => ($chr == 232) OR ($chr == 233) OR ($chr == 234) OR ($chr == 241),
      ENC_ASCII_EXT => ($chr >= 128) AND ($chr <= 255),
      ENC_ASCII_NUM => ($chr >= 48) AND ($chr <= 57),
      default => $status,
  };
		return $status;
	}
