...
class TCPDF_COLORS {
	...
	public static function convertHTMLColorToDec($hcolor, &$spotc, $defcol=['R'=>128, 'G'=>128, 'B'=>128]) {
		...
		switch (strlen((string) $color_code)) {
			case 3: {
				// 3-digit RGB hexadecimal representation
				$r = substr((string) $color_code, 0, 1);
				$g = substr((string) $color_code, 1, 1);
				$b = substr((string) $color_code, 2, 1);
				$returncolor = [];
			...
			case 6: {
				...
				$returncolor = [];
				$returncolor['R'] = max(0, min(255, hexdec(substr((string) $color_code, 0, 2))));
				$returncolor['G'] = max(0, min(255, hexdec(substr((string) $color_code, 2, 2))));
				$returncolor['B'] = max(0, min(255, hexdec(substr((string) $color_code, 4, 2))));
				break;
			}
			case 8: {
				// 8-digit CMYK hexadecimal representation
				$returncolor = [];
				$returncolor['C'] = max(0, min(100, round(hexdec(substr((string) $color_code, 0, 2)) / 2.55)));
				$returncolor['M'] = max(0, min(100, round(hexdec(substr((string) $color_code, 2, 2)) / 2.55)));
				$returncolor['Y'] = max(0, min(100, round(hexdec(substr((string) $color_code, 4, 2)) / 2.55)));
				$returncolor['K'] = max(0, min(100, round(hexdec(substr((string) $color_code, 6, 2)) / 2.55)));
				break;
			}
			default: {
				$returncolor = $defcol;
