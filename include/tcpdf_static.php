	...
	* @since 5.9.121 (2011-09-28)
	...
	public static function isValidCSSSelectorForTag($dom, $key, $selector) {
		...
		if (isset($dom[$key]['attribute']['class']) AND !empty($dom[$key]['attribute']['class'])) {
			$class = explode(' ', strtolower((string) $dom[$key]['attribute']['class']));
		}
		$id = '';
		if (isset($dom[$key]['attribute']['id']) AND !empty($dom[$key]['attribute']['id'])) {
			$id = strtolower((string) $dom[$key]['attribute']['id']);
		}
		$selector = preg_replace('/([\>\+\~\s]{1})([\.]{1})([^\>\+\~\s]*)/si', '\\1*.\\3', $selector);
		$matches = [];
		if (preg_match_all('/([\>\+\~\s]{1})([a-zA-Z0-9\*]+)([^\>\+\~\s]*)/si', (string) $selector, $matches, PREG_PATTERN_ORDER | PREG_OFFSET_CAPTURE) > 0) {
			...
			if (($lasttag == '*') OR ($lasttag == $tag)) {
							...
							if (preg_match('/\[([a-zA-Z0-9]*)[\s]*([\~\^\$\*\|\=]*)[\s]*["]?([^"\]]*)["]?\]/i', $attrib, $attrmatch) > 0) {
								...
								if (isset($dom[$key]['attribute'][$att])) {
									switch ($attrmatch[2]) {
										...
										case '~=': {
											if (in_array($val, explode(' ', (string) $dom[$key]['attribute'][$att]))) {
												$valid = true;
											}
											break;
										}
										case '^=': {
											if (str_starts_with((string) $dom[$key]['attribute'][$att], $val)) {
												$valid = true;
											}
											break;
										}
										case '$=': {
											if (str_ends_with((string) $dom[$key]['attribute'][$att], $val)) {
												$valid = true;
											}
											break;
										}
										case '*=': {
											if (str_contains((string) $dom[$key]['attribute'][$att], $val)) {
												$valid = true;
											}
											break;
										}
										case '|=': {
											if ($dom[$key]['attribute'][$att] == $val) {
												$valid = true;
											} elseif (preg_match('/'.$val.'[\-]{1}/i', (string) $dom[$key]['attribute'][$att]) > 0) {
												$valid = true;
											}
											break;
										}
				...
				if ($valid AND ($offset > 0)) {
					...
					$selector = substr((string) $selector, 0, $offset);
					switch ($operator) {
	...
	public static function getTagStyleFromCSSarray($css) {
		...
		foreach ($css as $style) {
			...
			$csscmds = explode(';', (string) $style['c']);
			foreach ($csscmds as $cmd) {
				if (!empty($cmd)) {
					$pos = strpos($cmd, ':');
					if ($pos !== false) {
						$cmd = substr($cmd, 0, ($pos + 1));
						if (str_contains($tagstyle, $cmd)) {
	...
	public static function getHyphenPatternsFromTEX($file) {
		...
		preg_match('/\\\\patterns\{([^\}]*)\}/i', (string) $data, $matches);
	...
	public static function pregSplit($pattern, $modifiers, $subject, $limit=NULL, $flags=NULL) {
		...
		if ((!str_contains($modifiers, 'u')) OR (count(preg_split('//u', "\n\t", -1, PREG_SPLIT_NO_EMPTY)) == 2)) {
	...
	public static function fileGetContents($file) {
		...
		if (isset($_SERVER['SCRIPT_URI'])
		    ...
		    $urldata = @parse_url((string) $_SERVER['SCRIPT_URI']);
	...
	public static $page_formats = [
     ...
     'C0'                     => [2599.370, 3676.535],
     // = (  917 x 1297 ) mm  = ( 36.10 x 51.06 ) in
     'C1'                     => [1836.850, 2599.370],
     // = (  648 x 917  ) mm  = ( 25.51 x 36.10 ) in
     'C2'                     => [1298.268, 1836.850],
     ...
     // SIS 014711 E Series
     'E0'                     => [2491.654, 3517.795],
     // = (  879 x 1241 ) mm  = ( 34.61 x 48.86 ) in
     'E1'                     => [1757.480, 2491.654],
     // = (  620 x 879  ) mm  = ( 24.41 x 34.61 ) in
     'E2'                     => [1247.244, 1757.480],
     ...
     // SIS 014711 G Series
     'G0'                     => [2715.591, 3838.110],
     // = (  958 x 1354 ) mm  = ( 37.72 x 53.31 ) in
     'G1'                     => [1919.055, 2715.591],
     // = (  677 x 958  ) mm  = ( 26.65 x 37.72 ) in
     'G2'                     => [1357.795, 1919.055],
     ...
     // ISO Press
     'RA0'                    => [2437.795, 3458.268],
     // = (  860 x 1220 ) mm  = ( 33.86 x 48.03 ) in
     'RA1'                    => [1729.134, 2437.795],
     ...
     '10R'                    => [720.000, 864.567],
     // = (  254 x 305  ) mm  = ( 10.00 x 12.01 ) in
     '4PW'                    => [720.000, 1080.000],
     // = (  254 x 381  ) mm  = ( 10.00 x 15.00 ) in
     'S10R'                   => [720.000, 1080.000],
     // = (  254 x 381  ) mm  = ( 10.00 x 15.00 ) in
     '11R'                    => [790.866, 1009.134],
     // = (  279 x 356  ) mm  = ( 10.98 x 14.02 ) in
     'S11R'                   => [790.866, 1224.567],
     // = (  279 x 432  ) mm  = ( 10.98 x 17.01 ) in
     '12R'                    => [864.567, 1080.000],
     // = (  305 x 381  ) mm  = ( 12.01 x 15.00 ) in
     'S12R'                   => [864.567, 1292.598],
     // = (  305 x 456  ) mm  = ( 12.01 x 17.95 ) in
     // Common Newspaper Sizes
     'NEWSPAPER_BROADSHEET'   => [2125.984, 1700.787],
     // = (  750 x 600  ) mm  = ( 29.53 x 23.62 ) in
     'NEWSPAPER_BERLINER'     => [1332.283, 892.913],
     // = (  470 x 315  ) mm  = ( 18.50 x 12.40 ) in
     'NEWSPAPER_TABLOID'      => [1218.898, 793.701],
     // = (  430 x 280  ) mm  = ( 16.93 x 11.02 ) in
     'NEWSPAPER_COMPACT'      => [1218.898, 793.701],
     // = (  430 x 280  ) mm  = ( 16.93 x 11.02 ) in
     // Business Cards
     'CREDIT_CARD'            => [153.014, 242.646],
     // = (   54 x 86   ) mm  = (  2.13 x 3.37  ) in
     'BUSINESS_CARD'          => [153.014, 242.646],
     // = (   54 x 86   ) mm  = (  2.13 x 3.37  ) in
     'BUSINESS_CARD_ISO7810'  => [153.014, 242.646],
     // = (   54 x 86   ) mm  = (  2.13 x 3.37  ) in
     'BUSINESS_CARD_ISO216'   => [147.402, 209.764],
     // = (   52 x 74   ) mm  = (  2.05 x 2.91  ) in
     'BUSINESS_CARD_IT'       => [155.906, 240.945],
     // = (   55 x 85   ) mm  = (  2.17 x 3.35  ) in
     'BUSINESS_CARD_UK'       => [155.906, 240.945],
     // = (   55 x 85   ) mm  = (  2.17 x 3.35  ) in
     'BUSINESS_CARD_FR'       => [155.906, 240.945],
     // = (   55 x 85   ) mm  = (  2.17 x 3.35  ) in
     'BUSINESS_CARD_DE'       => [155.906, 240.945],
     // = (   55 x 85   ) mm  = (  2.17 x 3.35  ) in
     'BUSINESS_CARD_ES'       => [155.906, 240.945],
     // = (   55 x 85   ) mm  = (  2.17 x 3.35  ) in
     'BUSINESS_CARD_CA'       => [144.567, 252.283],
     // = (   51 x 89   ) mm  = (  2.01 x 3.50  ) in
     'BUSINESS_CARD_US'       => [144.567, 252.283],
     // = (   51 x 89   ) mm  = (  2.01 x 3.50  ) in
     'BUSINESS_CARD_JP'       => [155.906, 257.953],
     // = (   55 x 91   ) mm  = (  2.17 x 3.58  ) in
     'BUSINESS_CARD_HK'       => [153.071, 255.118],
     // = (   54 x 90   ) mm  = (  2.13 x 3.54  ) in
     'BUSINESS_CARD_AU'       => [155.906, 255.118],
     // = (   55 x 90   ) mm  = (  2.17 x 3.54  ) in
     'BUSINESS_CARD_DK'       => [155.906, 255.118],
     // = (   55 x 90   ) mm  = (  2.17 x 3.54  ) in
     'BUSINESS_CARD_SE'       => [155.906, 255.118],
     // = (   55 x 90   ) mm  = (  2.17 x 3.54  ) in
     'BUSINESS_CARD_RU'       => [141.732, 255.118],
     // = (   50 x 90   ) mm  = (  1.97 x 3.54  ) in
     'BUSINESS_CARD_CZ'       => [141.732, 255.118],
     // = (   50 x 90   ) mm  = (  1.97 x 3.54  ) in
     'BUSINESS_CARD_FI'       => [141.732, 255.118],
     // = (   50 x 90   ) mm  = (  1.97 x 3.54  ) in
     'BUSINESS_CARD_HU'       => [141.732, 255.118],
     // = (   50 x 90   ) mm  = (  1.97 x 3.54  ) in
     'BUSINESS_CARD_IL'       => [141.732, 255.118],
     // = (   50 x 90   ) mm  = (  1.97 x 3.54  ) in
     // Billboards
     '4SHEET'                 => [2880.000, 4320.000],
     // = ( 1016 x 1524 ) mm  = ( 40.00 x 60.00 ) in
     '6SHEET'                 => [3401.575, 5102.362],
     // = ( 1200 x 1800 ) mm  = ( 47.24 x 70.87 ) in
     '12SHEET'                => [8640.000, 4320.000],
     // = ( 3048 x 1524 ) mm  = (120.00 x 60.00 ) in
     '16SHEET'                => [5760.000, 8640.000],
     // = ( 2032 x 3048 ) mm  = ( 80.00 x 120.00) in
     '32SHEET'                => [11520.000, 8640.000],
     // = ( 4064 x 3048 ) mm  = (160.00 x 120.00) in
     '48SHEET'                => [17280.000, 8640.000],
     // = ( 6096 x 3048 ) mm  = (240.00 x 120.00) in
     '64SHEET'                => [23040.000, 8640.000],
     // = ( 8128 x 3048 ) mm  = (320.00 x 120.00) in
     '96SHEET'                => [34560.000, 8640.000],
     // = (12192 x 3048 ) mm  = (480.00 x 120.00) in
     // -- Old European Sizes
     // - Old Imperial English Sizes
     'EN_EMPEROR'             => [3456.000, 5184.000],
     // = ( 1219 x 1829 ) mm  = ( 48.00 x 72.00 ) in
     'EN_ANTIQUARIAN'         => [2232.000, 3816.000],
     // = (  787 x 1346 ) mm  = ( 31.00 x 53.00 ) in
     'EN_GRAND_EAGLE'         => [2070.000, 3024.000],
     // = (  730 x 1067 ) mm  = ( 28.75 x 42.00 ) in
     'EN_DOUBLE_ELEPHANT'     => [1926.000, 2880.000],
     // = (  679 x 1016 ) mm  = ( 26.75 x 40.00 ) in
     'EN_ATLAS'               => [1872.000, 2448.000],
     // = (  660 x 864  ) mm  = ( 26.00 x 34.00 ) in
     'EN_COLOMBIER'           => [1692.000, 2484.000],
     // = (  597 x 876  ) mm  = ( 23.50 x 34.50 ) in
     'EN_ELEPHANT'            => [1656.000, 2016.000],
     // = (  584 x 711  ) mm  = ( 23.00 x 28.00 ) in
     'EN_DOUBLE_DEMY'         => [1620.000, 2556.000],
     // = (  572 x 902  ) mm  = ( 22.50 x 35.50 ) in
     'EN_IMPERIAL'            => [1584.000, 2160.000],
     // = (  559 x 762  ) mm  = ( 22.00 x 30.00 ) in
     'EN_PRINCESS'            => [1548.000, 2016.000],
     // = (  546 x 711  ) mm  = ( 21.50 x 28.00 ) in
     'EN_CARTRIDGE'           => [1512.000, 1872.000],
     // = (  533 x 660  ) mm  = ( 21.00 x 26.00 ) in
     'EN_DOUBLE_LARGE_POST'   => [1512.000, 2376.000],
     // = (  533 x 838  ) mm  = ( 21.00 x 33.00 ) in
     'EN_ROYAL'               => [1440.000, 1800.000],
     // = (  508 x 635  ) mm  = ( 20.00 x 25.00 ) in
     'EN_SHEET'               => [1404.000, 1692.000],
     // = (  495 x 597  ) mm  = ( 19.50 x 23.50 ) in
     'EN_HALF_POST'           => [1404.000, 1692.000],
     // = (  495 x 597  ) mm  = ( 19.50 x 23.50 ) in
     'EN_SUPER_ROYAL'         => [1368.000, 1944.000],
     // = (  483 x 686  ) mm  = ( 19.00 x 27.00 ) in
     'EN_DOUBLE_POST'         => [1368.000, 2196.000],
     // = (  483 x 775  ) mm  = ( 19.00 x 30.50 ) in
     'EN_MEDIUM'              => [1260.000, 1656.000],
     // = (  445 x 584  ) mm  = ( 17.50 x 23.00 ) in
     'EN_DEMY'                => [1260.000, 1620.000],
     // = (  445 x 572  ) mm  = ( 17.50 x 22.50 ) in
     'EN_LARGE_POST'          => [1188.000, 1512.000],
     // = (  419 x 533  ) mm  = ( 16.50 x 21.00 ) in
     'EN_COPY_DRAUGHT'        => [1152.000, 1440.000],
     // = (  406 x 508  ) mm  = ( 16.00 x 20.00 ) in
     'EN_POST'                => [1116.000, 1386.000],
     // = (  394 x 489  ) mm  = ( 15.50 x 19.25 ) in
     'EN_CROWN'               => [1080.000, 1440.000],
     // = (  381 x 508  ) mm  = ( 15.00 x 20.00 ) in
     'EN_PINCHED_POST'        => [1062.000, 1332.000],
     // = (  375 x 470  ) mm  = ( 14.75 x 18.50 ) in
     'EN_BRIEF'               => [972.000, 1152.000],
     // = (  343 x 406  ) mm  = ( 13.50 x 16.00 ) in
     'EN_FOOLSCAP'            => [972.000, 1224.000],
     // = (  343 x 432  ) mm  = ( 13.50 x 17.00 ) in
     'EN_SMALL_FOOLSCAP'      => [954.000, 1188.000],
     // = (  337 x 419  ) mm  = ( 13.25 x 16.50 ) in
     'EN_POTT'                => [900.000, 1080.000],
     // = (  318 x 381  ) mm  = ( 12.50 x 15.00 ) in
     // - Old Imperial Belgian Sizes
     'BE_GRAND_AIGLE'         => [1984.252, 2948.031],
     // = (  700 x 1040 ) mm  = ( 27.56 x 40.94 ) in
     'BE_COLOMBIER'           => [1757.480, 2409.449],
     // = (  620 x 850  ) mm  = ( 24.41 x 33.46 ) in
     'BE_DOUBLE_CARRE'        => [1757.480, 2607.874],
     // = (  620 x 920  ) mm  = ( 24.41 x 36.22 ) in
     'BE_ELEPHANT'            => [1746.142, 2182.677],
     // = (  616 x 770  ) mm  = ( 24.25 x 30.31 ) in
     'BE_PETIT_AIGLE'         => [1700.787, 2381.102],
     // = (  600 x 840  ) mm  = ( 23.62 x 33.07 ) in
     'BE_GRAND_JESUS'         => [1559.055, 2069.291],
     // = (  550 x 730  ) mm  = ( 21.65 x 28.74 ) in
     'BE_JESUS'               => [1530.709, 2069.291],
     // = (  540 x 730  ) mm  = ( 21.26 x 28.74 ) in
     'BE_RAISIN'              => [1417.323, 1842.520],
     // = (  500 x 650  ) mm  = ( 19.69 x 25.59 ) in
     'BE_GRAND_MEDIAN'        => [1303.937, 1714.961],
     // = (  460 x 605  ) mm  = ( 18.11 x 23.82 ) in
     'BE_DOUBLE_POSTE'        => [1233.071, 1601.575],
     // = (  435 x 565  ) mm  = ( 17.13 x 22.24 ) in
     'BE_COQUILLE'            => [1218.898, 1587.402],
     // = (  430 x 560  ) mm  = ( 16.93 x 22.05 ) in
     'BE_PETIT_MEDIAN'        => [1176.378, 1502.362],
     // = (  415 x 530  ) mm  = ( 16.34 x 20.87 ) in
     'BE_RUCHE'               => [1020.472, 1303.937],
     // = (  360 x 460  ) mm  = ( 14.17 x 18.11 ) in
     'BE_PROPATRIA'           => [977.953, 1218.898],
     // = (  345 x 430  ) mm  = ( 13.58 x 16.93 ) in
     'BE_LYS'                 => [898.583, 1125.354],
     // = (  317 x 397  ) mm  = ( 12.48 x 15.63 ) in
     'BE_POT'                 => [870.236, 1088.504],
     // = (  307 x 384  ) mm  = ( 12.09 x 15.12 ) in
     'BE_ROSETTE'             => [765.354, 983.622],
     // = (  270 x 347  ) mm  = ( 10.63 x 13.66 ) in
     // - Old Imperial French Sizes
     'FR_UNIVERS'             => [2834.646, 3685.039],
     // = ( 1000 x 1300 ) mm  = ( 39.37 x 51.18 ) in
     'FR_DOUBLE_COLOMBIER'    => [2551.181, 3571.654],
     // = (  900 x 1260 ) mm  = ( 35.43 x 49.61 ) in
     'FR_GRANDE_MONDE'        => [2551.181, 3571.654],
     // = (  900 x 1260 ) mm  = ( 35.43 x 49.61 ) in
     'FR_DOUBLE_SOLEIL'       => [2267.717, 3401.575],
     // = (  800 x 1200 ) mm  = ( 31.50 x 47.24 ) in
     'FR_DOUBLE_JESUS'        => [2154.331, 3174.803],
     // = (  760 x 1120 ) mm  = ( 29.92 x 44.09 ) in
     'FR_GRAND_AIGLE'         => [2125.984, 3004.724],
     // = (  750 x 1060 ) mm  = ( 29.53 x 41.73 ) in
     'FR_PETIT_AIGLE'         => [1984.252, 2664.567],
     // = (  700 x 940  ) mm  = ( 27.56 x 37.01 ) in
     'FR_DOUBLE_RAISIN'       => [1842.520, 2834.646],
     // = (  650 x 1000 ) mm  = ( 25.59 x 39.37 ) in
     'FR_JOURNAL'             => [1842.520, 2664.567],
     // = (  650 x 940  ) mm  = ( 25.59 x 37.01 ) in
     'FR_COLOMBIER_AFFICHE'   => [1785.827, 2551.181],
     // = (  630 x 900  ) mm  = ( 24.80 x 35.43 ) in
     'FR_DOUBLE_CAVALIER'     => [1757.480, 2607.874],
     // = (  620 x 920  ) mm  = ( 24.41 x 36.22 ) in
     'FR_CLOCHE'              => [1700.787, 2267.717],
     // = (  600 x 800  ) mm  = ( 23.62 x 31.50 ) in
     'FR_SOLEIL'              => [1700.787, 2267.717],
     // = (  600 x 800  ) mm  = ( 23.62 x 31.50 ) in
     'FR_DOUBLE_CARRE'        => [1587.402, 2551.181],
     // = (  560 x 900  ) mm  = ( 22.05 x 35.43 ) in
     'FR_DOUBLE_COQUILLE'     => [1587.402, 2494.488],
     // = (  560 x 880  ) mm  = ( 22.05 x 34.65 ) in
     'FR_JESUS'               => [1587.402, 2154.331],
     // = (  560 x 760  ) mm  = ( 22.05 x 29.92 ) in
     'FR_RAISIN'              => [1417.323, 1842.520],
     // = (  500 x 650  ) mm  = ( 19.69 x 25.59 ) in
     'FR_CAVALIER'            => [1303.937, 1757.480],
     // = (  460 x 620  ) mm  = ( 18.11 x 24.41 ) in
     'FR_DOUBLE_COURONNE'     => [1303.937, 2040.945],
     // = (  460 x 720  ) mm  = ( 18.11 x 28.35 ) in
     'FR_CARRE'               => [1275.591, 1587.402],
     // = (  450 x 560  ) mm  = ( 17.72 x 22.05 ) in
     'FR_COQUILLE'            => [1247.244, 1587.402],
     // = (  440 x 560  ) mm  = ( 17.32 x 22.05 ) in
     'FR_DOUBLE_TELLIERE'     => [1247.244, 1927.559],
     // = (  440 x 680  ) mm  = ( 17.32 x 26.77 ) in
     'FR_DOUBLE_CLOCHE'       => [1133.858, 1700.787],
     // = (  400 x 600  ) mm  = ( 15.75 x 23.62 ) in
     'FR_DOUBLE_POT'          => [1133.858, 1757.480],
     // = (  400 x 620  ) mm  = ( 15.75 x 24.41 ) in
     'FR_ECU'                 => [1133.858, 1474.016],
     // = (  400 x 520  ) mm  = ( 15.75 x 20.47 ) in
     'FR_COURONNE'            => [1020.472, 1303.937],
     // = (  360 x 460  ) mm  = ( 14.17 x 18.11 ) in
     'FR_TELLIERE'            => [963.780, 1247.244],
     // = (  340 x 440  ) mm  = ( 13.39 x 17.32 ) in
     'FR_POT'                 => [878.740, 1133.858],
 ];
	 ...
	 * @public static
	 */
	public static function getPageSizeFromFormat(mixed $format) {
		return self::$page_formats[$format] ?? self::$page_formats['A4'];
	}
	...
	public static function getPageLayoutMode($layout='SinglePage') {
		$layout_mode = match ($layout) {
      'default', 'single', 'SinglePage' => 'SinglePage',
      'continuous', 'OneColumn' => 'OneColumn',
      'two', 'TwoColumnLeft' => 'TwoColumnLeft',
